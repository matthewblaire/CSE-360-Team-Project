...
public class ViewMessagesList {
	...
	protected static ListView<String> listview_MyMessageThreads = new ListView<>();
	...
	private static ViewMessagesList theView;
	...
	private static Scene theMessagesListScene;
	 ...
	 */
	public static void displayMessagesList(Stage ps, User user) {

		theStage = ps;
		theUser  = user;

		if (theView == null) theView = new ViewMessagesList();
		theStage.setTitle("CSE 360 Foundations: My Messages");
		theStage.setScene(theMessagesListScene);

		// Dynamic refresh
		label_UserDetails.setText("User: " + theUser.getUserName());
		listview_MyMessageThreads.setItems(FXCollections.observableArrayList());
		ControllerMessagesList.listOfDistinctOtherUsers.clear();

		// Populate the list immediately on every visit
		ControllerMessagesList.doLoadMyMessages();
	 ...
	 * displayMyPosts(). </p>
	 */
	private ViewMessagesList() {

		theRootPane    = new Pane();
		theMessagesListScene = new Scene(theRootPane, width, height);
		...
		button_Return.setOnAction((_) -> { ControllerMessagesList.performReturn(); });
		...
		button_Refresh.setOnAction((_) -> { ControllerMessagesList.doLoadMyMessages(); });



		
	    // Double click handler for Posts
		listview_MyMessageThreads.setOnMouseClicked(event -> {
	        if (event.getClickCount() == 2 && !listview_MyMessageThreads.getSelectionModel().isEmpty()) {
	            Object selectedItem = listview_MyMessageThreads.getSelectionModel().getSelectedItem();
	            ControllerMessagesList.doHandleMessageThreadDoubleClick(selectedItem, theStage, theUser);
	            event.consume();
	        }
	    });
		
		listview_MyMessageThreads.setStyle("-fx-font-family: 'Dialog'; -fx-font-size: 13px;");
		listview_MyMessageThreads.setLayoutX(20);
		listview_MyMessageThreads.setLayoutY(138);
		listview_MyMessageThreads.setPrefWidth(760);
		listview_MyMessageThreads.setPrefHeight(340);


		// ============================ Area 3: Footer ============================

		setupButtonUI(button_Logout, "Dialog", 18, 250, Pos.CENTER, 20,  540);
		button_Logout.setOnAction((_) -> { ControllerMessagesList.performLogout(); });

		setupButtonUI(button_Quit,   "Dialog", 18, 250, Pos.CENTER, 300, 540);
		button_Quit.setOnAction((_) -> { ControllerMessagesList.performQuit(); });


		// Add all widgets to the root Pane
		theRootPane.getChildren().addAll(
				label_PageTitle, label_UserDetails, button_Return, line_Separator1,
				label_PostCount, button_Refresh, listview_MyMessageThreads,
